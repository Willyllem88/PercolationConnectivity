...
file_list <- list.files("../data/triangular", pattern = "*.csv", full.names = TRUE)
...
for (file in file_list) {
  ...
  plot <- ggplot(data, aes(x = p, y = connectedGraphsPercentatge, color = as.factor(n))) +
    ...
    labs(
      title = paste("Percentatge de grafs triangulars connexos segons rows i p"),
      x = "Probabilitat de fallida (p)",
      y = "Percentatge de grafs conexos",
      color = "Número de files (rows)"
    ) +
  ...
  ggsave(filename = paste0("../data/triangular/", file_name, ".png"), bg = "white", plot = plot, width = 8, height = 6)
}