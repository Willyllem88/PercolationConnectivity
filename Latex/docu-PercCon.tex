...
\begin{document}
	...
	\begin{table}[H]
		...
		\begin{tabular}{|c|c|c|}
			...
			140 & 0.07 & 0.15 \\ \hline
		\end{tabular}
		\caption{Comparació del \textit{threshold} entre els grafs quadrats i triangulars}
		\label{tab:comparacio_threshold_square_triangular}
	\end{table}
	
	La \textit{Taula \ref{tab:comparacio_threshold_square_triangular}} ens mostra una comparació de $threshold$ en grafs amb el mateix nombre de files però de diferent tipus, graelles quadrades o triangulars. Com ja hem anat suposant però amb la taula podem confirmar, en les graelles quadrades el moment on apareix la transició de fase és més petit que en les graelles triangulars. Amb aquestes dades podem confirmar la nostra hipòtesi d'experimentació on crèiem que el fet de tenir graelles on els nodes tenen més arestes influiria en el valor $p$ on apareix la transició de fase.
