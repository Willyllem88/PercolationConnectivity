...
file_list <- list.files("../data/square", pattern = "*.csv", full.names = TRUE)
...
for (file in file_list) {
  ...
  plot <- ggplot(data, aes(x = p, y = connectedGraphsPercentatge, color = as.factor(n))) +
    ...
    labs(
      title = paste("Percentatge de grafs quadrats connexos segons m i p"),
      x = "Probabilitat de fallida (p)",
      y = "Percentatge de grafs conexos",
      color = "Número de files (m)"
    ) +
  ...
  ggsave(filename = paste0("../data/square/", file_name, ".png"), bg = "white", plot = plot, width = 8, height = 6)
}