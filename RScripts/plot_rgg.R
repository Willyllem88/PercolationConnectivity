...
file_list <- list.files("../data/rgg", pattern = "*.csv", full.names = TRUE)
...
for (file in file_list) {
  ...
  plot <- ggplot(data, aes(x = p, y = connectedGraphsPercentatge, color = as.factor(n))) +
    ...
    labs(
      title = paste("Percentatge de grafs geomètrics aleatoris connexos segons n i p"),
  ...
  ggsave(filename = paste0("../data/rgg/", file_name, ".png"), bg = "white", plot = plot, width = 8, height = 6)
}